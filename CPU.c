...
int while_loop()
{
	...
	for(i=index; i< count; i++)
	{
		...
		if(!(strcmp(op.opcode,"bgt")))
		{
			...
			equal = bgt(); //if condition is trure return 1 else 0
			if(equal==1)
				continue;
			else
				return; 
...

{
	...
	for(i=0; i<10; i++)
	{
		instr_mem[i]=(char *)malloc(80*sizeof(char));
	}
	i=0;
